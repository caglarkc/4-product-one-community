...
`backend/` normal auth ve hesap yönetimi API'lerini; `frontend/` gerçek API bağlantılı Next.js ekranlarını içerir. Redis'in oturum, sayaç ve geçici token sorumlulukları uygulama öncesi [run teknik kararlarında](../.orchestrator/runs/first-auth/technical-decisions.md) kaydedildi. İlk auth görevinde django-allauth/OAuth henüz kurulmamıştı; sonraki teslimlerde Google/GitHub ve seçili repo GitHub App entegrasyonları uygulanmıştır (bkz. deployment.md). Kalıcı kimlik modeli ileride sağlayıcı bağlantılarının ayrı tutulmasına uygundur.

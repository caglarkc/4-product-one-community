...
describe('normal auth forms',()=>{
 ...
 it('shows unverified-email reminder and working account target',async()=>{vi.stubGlobal('fetch',vi.fn().mockResolvedValue(Response.json({user:{full_name:'Üye',email_verified:false}})));render(<AccountStatus/>);expect(await screen.findByText('E-posta adresiniz henüz doğrulanmadı.')).toBeInTheDocument();expect(screen.getByRole('link',{name:'Hesabımı yönet'})).toHaveAttribute('href','/hesap');});
});

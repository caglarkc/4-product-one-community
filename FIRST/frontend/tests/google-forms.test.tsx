...
describe('Google registration',()=>{
 ...
 const fetcher=vi.fn().mockResolvedValueOnce(Response.json(pending)).mockResolvedValueOnce(Response.json({csrfToken:'csrf'})).mockResolvedValueOnce(Response.json({user:{id:1}}));vi.stubGlobal('fetch',fetcher);render(<GoogleSignup/>);
...
it('does not verify email on opening the page; explicit confirmation posts key and routes known status',async()=>{
 ...
 const fetcher=vi.fn().mockResolvedValueOnce(Response.json({csrfToken:'csrf'})).mockResolvedValueOnce(Response.json({status:'profile_required'}));vi.stubGlobal('fetch',fetcher);render(<VerifyGoogleEmail token="opaque-proof"/>);
 expect(fetcher).not.toHaveBeenCalled();fireEvent.click(screen.getByRole('button',{name:'Doğrula ve devam et'}));await waitFor(()=>expect(nav.replace).toHaveBeenCalledWith('/kayit/google'));expect(fetcher.mock.calls[1][0]).toBe('/api/auth/google/email/verify/');expect(JSON.parse(fetcher.mock.calls[1][1].body)).toEqual({key:'opaque-proof'});
});

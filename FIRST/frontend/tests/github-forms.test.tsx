...
const GitHubSignup=()=> <SocialSignup provider="github"/>;
import {GitHubButton} from '../src/components/github-auth';
...
describe('GitHub registration',()=>{
 ...
 expect(JSON.parse(fetcher.mock.calls[2][1].body)).toEqual({full_name:'GitHub Üye',username:'üye',birth_date:'2000-01-01',phone:'',gender:'unspecified'});
 });
 it('offers restart for expired pending signup',async()=>{
 vi.stubGlobal('fetch',vi.fn().mockResolvedValue(Response.json({detail:'Kayıt süresi doldu.'},{status:400})));render(<GitHubSignup/>);expect(await screen.findByRole('alert')).toHaveTextContent('Kayıt süresi doldu');expect(screen.getByRole('link',{name:'GitHub ile yeniden giriş yap'})).toHaveAttribute('href','/giris');
 });
 it('rejects non-GitHub redirect and permits retry',async()=>{
 const fetcher=vi.fn().mockResolvedValueOnce(Response.json({providers:{github:true}})).mockResolvedValueOnce(Response.json({csrfToken:'csrf'})).mockResolvedValueOnce(Response.json({authorization_url:'https://evil.example/'}));vi.stubGlobal('fetch',fetcher);render(<GitHubButton remember/>);
 fireEvent.click(await screen.findByRole('button',{name:'GitHub ile devam et'}));expect(await screen.findByRole('alert')).toHaveTextContent('doğrulanamadı');expect(JSON.parse(fetcher.mock.calls[2][1].body)).toEqual({remember_me:true,purpose:'login'});expect(screen.getByRole('button')).not.toBeDisabled();
 });
});
...
it('requires reauthentication before link and allows an explicit retry after password proof',async()=>{
 const {ReauthenticationContext}=await import('../src/components/account-form');
 const fetcher=vi.fn().mockResolvedValueOnce(Response.json({providers:{github:true}}))
 .mockResolvedValueOnce(Response.json({csrfToken:'csrf'})).mockResolvedValueOnce(Response.json({detail:'Kimliğinizi doğrulayın.',code:'reauthentication_required'},{status:403}))
 .mockResolvedValueOnce(Response.json({csrfToken:'csrf'})).mockResolvedValueOnce(Response.json({detail:'Doğrulandı.'}));
 vi.stubGlobal('fetch',fetcher);render(<ReauthenticationContext.Provider value={{google:false,password:true}}><GitHubButton purpose="link"/></ReauthenticationContext.Provider>);
 fireEvent.click(await screen.findByRole('button',{name:'GitHub hesabımı bağla'}));
 const password=await screen.findByLabelText('Mevcut şifreniz');expect(screen.getByRole('button',{name:'GitHub hesabımı bağla'})).toBeDisabled();expect(JSON.parse(fetcher.mock.calls[2][1].body).purpose).toBe('link');
 fireEvent.change(password,{target:{value:'Password123!'}});fireEvent.click(screen.getByRole('button',{name:'Kimliğimi doğrula'}));
 await waitFor(()=>expect(screen.getByRole('button',{name:'GitHub hesabımı bağla'})).toBeEnabled());expect(fetcher).toHaveBeenCalledTimes(5);
});

it('offers password setup for GitHub-only sensitive actions, not unsupported provider reauth',async()=>{
 ...
 render(<ReauthenticationContext.Provider value={{google:false,password:false}}><AccountForm title="E-posta değiştir" path="email/change" submit="Kaydet"/></ReauthenticationContext.Provider>);
 fireEvent.submit(screen.getByRole('button',{name:'Kaydet'}).closest('form')!);
 expect(await screen.findByRole('link',{name:'e-posta bağlantısıyla bir şifre oluşturun'})).toHaveAttribute('href','/sifremi-unuttum');expect(screen.queryByLabelText('Mevcut şifreniz')).not.toBeInTheDocument();
});

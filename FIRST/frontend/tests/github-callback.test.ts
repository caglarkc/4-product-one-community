...
describe('GitHub callback boundary',()=>{
 ...
 const result=await GET(new Request('https://front.example/accounts/github/login/callback/?code=secret&state=one&next=https://evil.example&scope=email',{headers:{cookie:'sessionid=before'}}));
 ...
 expect(String(fetcher.mock.calls[0][0])).toBe('https://backend.example/api/auth/github/callback/?code=secret&state=one');expect(fetcher.mock.calls[0][1].headers.get('cookie')).toBe('sessionid=before');
 });
 ...
 expect((await GET(new Request('https://front.example/accounts/github/login/callback/?state=a&state=b'))).headers.get('location')).toBe('/giris?github_error=failed');expect(fetcher).not.toHaveBeenCalled();
 });
});
...
it('signs exact backend callback path while forwarding encoded query separately',async()=>{
 ...
 await GET(new Request('https://front.example/accounts/github/login/callback/?code=a%2Bb&state=c',{headers:{'x-trusted-client-ip':'192.0.2.1'}}));
 expect(fetcher.mock.calls[0][1].headers.get('X-First-Client-Signature')).toBe(createHmac('sha256',secret).update('192.0.2.1\n1800000000\nGET\n/api/auth/github/callback/').digest('hex'));
...
it('turns cancellation into a fixed safe error route',async()=>{
 ...
 const result=await GET(new Request('https://front.example/accounts/github/login/callback/?error=access_denied&state=one'));
 expect(result.headers.get('location')).toBe('/giris?github_error=cancelled');
});

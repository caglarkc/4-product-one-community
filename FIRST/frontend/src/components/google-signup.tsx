...
export function GoogleSignup() {
  ...
  useEffect(() => {let active = true;
    api<Pending>('google/signup').then(data => {if(active) setPending(data);}).catch(caught => {if(active) setError(caught);});
    return () => {active = false;};
  }, []);
  ...
  return <Surface className="form-page register-page"><PageHeading title="Kaydınızı tamamlayın" description="Google’dan gelen bilgileri kontrol edin ve eksik alanları doldurun. Şifre oluşturmanız gerekmiyor."/>
    {error && <Alert role="alert" tone="error"><p>{error.message}</p>{error.errors.non_field_errors?.map(text => <p key={text}>{text}</p>)}{(!pending || error.code === 'google_signup_expired') && <Link href="/giris">Google ile yeniden giriş yap</Link>}</Alert>}
    {!pending && !error && <Alert role="status">Google bilgileri yükleniyor…</Alert>}
    ...
    {pending && <form aria-busy={busy} onSubmit={async event => {
      ...
      try {await api('google/signup', payload); router.replace('/'); router.refresh();}
      ...
      {!pending.email_verified && <div><p className="form-hint">Kaydınızı tamamlamadan önce e-posta adresinizi doğrulayın. Bağlantıyı bu tarayıcıda açın; mevcut bir hesabınız varsa doğrulama sonrası o hesaba giriş yapılır.</p>
        <Button variant="secondary" disabled={busy} onClick={async event => {
          ...
          try {const result = await api<{detail: string}>('google/email/request', {email: input.value}); setMessage(result.detail);}
          catch(caught) {setError(caught as ApiError);}
          finally {lock.current = false; setBusy(false);}

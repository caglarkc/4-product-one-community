...
function MyProjectsContent(){
 ...
 <div className="project-grid">{projects?.map(project=><Surface key={project.id} className="project-card"><div className="project-card-heading"><h2><a href={`/projeler/${project.id}`}>{project.title}</a></h2><span className={`connection-status${project.is_active?' connection-status--connected':''}`}>{project.is_active?'Yayında':'Arşivde'}</span></div><ProjectContent project={project}/><div className="action-row"><ActionLink variant="secondary" href={`/projeler/${project.id}`}>Paylaşımı görüntüle</ActionLink><ActionLink variant="quiet" href={`/projelerim/${project.id}/duzenle`}>Düzenle</ActionLink></div></Surface>)}</div></div>;
}
...
function ProjectEditorContent({id}:{id?:string}){
 ...
 {error&&<Alert role="alert" tone="error"><p>{error}</p>{ready&&!busy&&<Button variant="secondary" onClick={()=>setRetry(value=>value+1)}>Yeniden dene</Button>}</Alert>} {!ready&&<Alert role="status">Paylaşım bilgileri yükleniyor…</Alert>}
 ...
 {user&&config&&(existing||preview)&&<Surface className="project-panel"><h2>{id?'Paylaşım bilgileri':'2. Paylaşımı gözden geçirin'}</h2><form aria-busy={busy} onSubmit={event=>{event.preventDefault();if(!eligible||(!id&&!confirmed))return;perform(async()=>{const body=id?{title,category,description}:{title,category,description,installation_id:preview!.repository.installation_id,repository_id:preview!.repository.id,readme_excerpt:includeExcerpt?preview!.readme_excerpt:''};const data=await api<{project:Project}>(id?`projects/${id}`:'projects',body,id?'PATCH':'POST');router.push(`/projeler/${data.project.id}`);});}}><fieldset disabled={busy}>
 ...
 <Button type="submit" loading={busy} disabled={!eligible||(!id&&!confirmed)}>{id?'Değişiklikleri kaydet':'Projeyi paylaş'}</Button></fieldset></form>

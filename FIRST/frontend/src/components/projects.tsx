...
export function ProjectEditor({id}:{id?:string}){
 ...
 const eligible=!!user?.email_verified&&!!user?.providers.includes('github');const install=githubUrl(status?.installation_url||null,true);
 ...
 {install&&<ActionLink href={install} target="_blank" rel="noopener noreferrer" variant="secondary">GitHub’da repo izinlerini seç</ActionLink>}
 <Button disabled={busy} onClick={()=>perform(async()=>{const result=await api<{authorization_url:string}>('projects/github/start',{});const target=new URL(result.authorization_url);if(target.origin!=='https://github.com'||target.pathname!=='/login/oauth/authorize'||target.username||target.password)throw new Error('GitHub bağlantı adresi doğrulanamadı.');window.location.assign(target.href);})}>{status.connected?'GitHub bağlantısını yenile':'GitHub repo erişimini bağla'}</Button>

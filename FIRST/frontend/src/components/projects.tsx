...
function ProjectEditorContent({id}:{id?:string}){
 ...
 {status.connected&&<><p className="field-help">{repositoryCacheCaption(cachedAt)}. Liste otomatik güncellenmez; yeni repolar ve izin değişiklikleri için yenileyin.</p><Button variant="quiet" loading={busy} disabled={busy} onClick={()=>{if(lock.current)return;setPreview(undefined);setConfirmed(false);void perform(async()=>{const data=await api<RepositoryCache>('projects/github/repositories',{});setRepositories(data.repositories);setCachedAt(data.cached_at ?? null);setSelected('');});}}>Repo listesini yenile</Button>
 ...
 {!id&&preview&&<>{!preview.preview_token&&<Alert tone="error">Paylaşım hazırlığı güncel değil. Paylaşımı hazırla adımını yeniden deneyin.</Alert>}<Alert><p>{preview.repository.private?'Gizli repo: Repo bağlantısı ve dosyalar yayımlanmaz. Aşağıdaki başlık, üst ve alt kategori, proje durumu, açıklama ve seçerseniz README özeti herkes tarafından görülebilir.':'Açık repo: Başlık, üst ve alt kategori, proje durumu, açıklama, seçerseniz README özeti ve GitHub bağlantısı herkes tarafından görülebilir.'}</p><p>Onayladığınız repo bilgileri ve README özeti FIRST’te kaydedilir. GitHub’daki sonraki değişiklikler otomatik olarak bu paylaşıma aktarılmaz.</p></Alert>
 ...
 <Button type="submit" loading={busy} disabled={!eligible||!taxonomySelected||(!id&&(!confirmed||!preview?.preview_token))}>{id?'Değişiklikleri kaydet':'Projeyi paylaş'}</Button></fieldset></form>

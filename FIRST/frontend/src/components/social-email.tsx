...
export function VerifySocialEmail({token, provider}: {token: string; provider: 'google' | 'github'}) {
  ...
  return <Surface className="form-page"><PageHeading title={`${label} girişi için e-postanızı doğrulayın`}/>
    {token ? <AccountForm<{status: string}> title="E-posta adresini onayla" path={`${provider}/email/verify`} extra={{key: token}} submit="Doğrula ve devam et" onSuccess={data => {
      ...
      router.replace(data.status === 'authenticated' ? (provider === 'github' ? '/github-kurulum?next=%2F' : '/') : `/kayit/${provider}`); router.refresh();

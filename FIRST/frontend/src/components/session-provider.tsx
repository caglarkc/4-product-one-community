...
export function SessionNavigation() {
  ...
  return <div className="navigation-group"><nav className="site-nav" aria-label="Ana gezinme">
    {status === 'loading' ? <span className="nav-status" role="status">Hesap yükleniyor…</span> : status === 'error' ? <Button variant="quiet" onClick={() => void refresh()}>Hesabı yeniden kontrol et</Button> : user ? <>
      {[['/', 'Projeler'], ['/ekipler', 'Ekipler'], ['/gorevler', 'Görevler'], ['/kisiler', 'Kişiler']].map(([href, label]) => <ActionLink key={href} href={href} variant="quiet" aria-current={current(href) ? 'page' : undefined}>{label}</ActionLink>)}
      <details className="nav-account"><summary>Hesabım</summary><div className="nav-account-links">{[['/projelerim', 'Projelerim'], ['/kaydedilenler', 'Kaydedilenler'], ['/basvurular', 'Başvurular'], ['/bildirimler', 'Bildirimler'], ['/hesap', 'Hesap ayarları']].map(([href,label])=><ActionLink key={href} href={href} variant="quiet" aria-current={current(href)?'page':undefined} onClick={event=>event.currentTarget.closest('details')?.removeAttribute('open')}>{label}</ActionLink>)}

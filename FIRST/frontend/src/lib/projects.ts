...
export type Project = ProjectSummary & {team?:{id:string;name:string}|null;readme_excerpt:string;is_private:boolean;repository_url:string|null;repository_name:string|null;is_active:boolean;visibility:string;applications_open:boolean;current_state:string;desired_outcome:string;issue_number:number|null;issue_status:string;is_owner:boolean;can_apply:boolean;owner_username:string};

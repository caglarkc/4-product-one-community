...
## Release completion — 25 September 2026

`/ekipler` and `/ekipler/[id]` consume the independent team API with discovery, membership/invitations, bookmarks, owner/admin management and paginated member/request/project collections. `/gorevler` consumes public Issue tasks with signed candidate cursors; project details add manual task attachment/creation and synchronization. The create form retains exact request IDs across ambiguous outcomes; explicit form release does not cancel an unknown GitHub operation. Private project detail instead offers a three-file snapshot showcase with owner browse, preview, explicit consent, replacement and removal. Visitors receive only safe authenticated JSON previews; Markdown/code/CSV remain inert, PDF pages and supported images are rasterized server-side.

Compact shared tokens use a 1280px maximum shell, 14px body, 22–28px page titles, 16–20px panel padding and 44px controls. Account links are grouped in a native disclosure menu. Feature routes use the same semantic UI components and responsive styles. Only source/diff review was performed in this release; historical test statements above do not validate these changes.
